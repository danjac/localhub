...
class TestGetLocalCommunitiesCount:
    def test_anonymous_user_if_no_other_community(self, community: Community):
        assert get_local_communities_count(AnonymousUser(), community) == 0

    def test_anonymous_user_if_other_community_public(
        self, community: Community
    ):
        CommunityFactory(public=True)
        assert get_local_communities_count(AnonymousUser(), community) == 1
    ...
    def test_user_if_no_other_memberships(
        ...
        assert (
            get_local_communities_count(member.member, member.community) == 0
        )
    ...
    def test_user_if_no_other_memberships_if_other_community(
        ...
        assert get_local_communities_count(member.member, community) == 1
    ...
    def test_user_if_other_memberships_if_other_community_not_public(
        ...
        assert (
            get_local_communities_count(member.member, member.community) == 1
        )
