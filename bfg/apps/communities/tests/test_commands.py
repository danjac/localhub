...
class TestCreateCommunity:
    ...
    def test_with_new_domain(self):
        call_command("createcommunity", "mydomain.bfg.social", "My Domain")
        community = Community.objects.first()
        assert community.name == "My Domain"
        assert community.domain == "mydomain.bfg.social"
        assert community.active

    def test_with_new_domain_and_nonexistent_user(self):
        with pytest.raises(CommandError):
            call_command(
                "createcommunity", "mydomain.bfg.social", "My Domain", admin="me",
            )
        community = Community.objects.first()
    ...
    def test_with_new_domain_and_admin(self, user):
        call_command(
            "createcommunity", "mydomain.bfg.social", "My Domain", admin=user.username,
        )
        community = Community.objects.first()
        assert community.name == "My Domain"
        assert community.domain == "mydomain.bfg.social"
